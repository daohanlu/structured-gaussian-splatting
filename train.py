...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    print('# test cameras', len(scene.getTrainCameras()))
    if args.freeze_xyz:
        gaussians.freeze_means = True
        assert gaussians.get_xyz.requires_grad == False
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, tsv_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
...
def prepare_output_and_logger(args, opt_args):
    ...
    with open(os.path.join(args.model_path, "opt_args"), 'w') as opt_log_f:
        opt_log_f.write(str(Namespace(**vars(opt_args))))
...
def training_report(tb_writer, tsv_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    if tb_writer:
...
if __name__ == "__main__":
    ...
    save_iters = [0, 1_000, 4_500, 7_000, 11_000, 15_000, 20_000, 25_000, 30_000]
